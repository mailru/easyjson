	...
	if *update {
		...
		if err != nil {
			...
			if stat.ModTime().After(fInfo.ModTime()) {
				// intermediate files are generated with the same name before the final version is created
				// so we need to check this file is actually the final one
				open, err := os.Open(outName)
				if err != nil {
					return err
				}
				line, err := bufio.NewReader(open).ReadString('\n')
				if err != nil {
					return err
				}
				if strings.Contains(line, "Code generated by easyjson for marshaling/unmarshaling.") {
